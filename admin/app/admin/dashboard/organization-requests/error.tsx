...
export default function OrganizationRequestsError({
  error,
  reset,
}: {
      ...
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="p-6">
          <div className="text-center">
            <h3 className="text-lg font-medium text-gray-900 mb-1">Organization Requests Error</h3>
            <p className="text-sm text-gray-500 mb-6">
              We encountered an issue while loading the organization requests. Please try again or contact support.
            </p>
            
            <div className="mb-6 p-3 bg-gray-50 rounded-md text-xs text-left overflow-auto max-h-32">
              <p className="font-mono text-red-600">{error.message || "An unknown error occurred"}</p>
              {error.digest && (
                <p className="font-mono text-gray-500 mt-2">Error ID: {error.digest}</p>
              )}
            </div>
